...
@guest
    ...
    <section class="section" style="margin-top:40px;">
        ...
        <div style="display:grid; grid-template-columns: 1.1fr 0.9fr; gap:24px;">
            <!-- Big card left - dynamic trending -->
            <div class="trending-big-card">
                @php $first = $trending->first(); @endphp
                @if($first)
                <div style="position:relative;">
                    <img src="{{ $first->image_url ?? 'https://images.unsplash.com/photo-1604908176997-125f25cc6f3d?q=80&w=800' }}" alt="{{ $first->title }}">
                ...
                <div style="padding:24px; flex:1; display:flex; flex-direction:column; justify-content:space-between;">
                    <div>
                        <div style="display:flex; gap:10px; font-size:11px; font-weight:700; color:#999; margin-bottom:8px;">
                            <span>{{ $first->total_time ?? '—' }} MINS</span>
                            <span>•</span>
                            <span>{{ ucfirst($first->difficulty ?? 'medium') }}</span>
                        </div>
                        <h3 style="font-size:22px; font-weight:800; color:#111; line-height:1.3;">{{ $first->title }}</h3>
                        <p style="color:var(--text-m); font-size:13px; margin-top:6px; line-height:1.5;">{{ \Illuminate\Support\Str::limit($first->description, 140) }}</p>
                    </div>
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-top:20px; padding-top:16px; border-top:1px solid #F0F0F0;">
                        <span style="font-size:12px; color:#777;"><i class="far fa-eye"></i> {{ $first->view_count ?? 0 }} views</span>
                        <span style="color:#FFA500; font-weight:800; font-size:13px;"><i class="fas fa-star"></i> {{ $first->rating_avg ?? '0.0' }}</span>
                    </div>
                </div>
                @else
                    <div style="padding:24px;">Belum ada resep trending saat ini.</div>
                @endif
            </div>
