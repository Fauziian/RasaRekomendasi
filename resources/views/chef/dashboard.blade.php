...
<div style="display:grid;grid-template-columns:repeat(4, 1fr);gap:20px;margin-bottom:28px">
    ...
    <div class="white-card" style="margin-bottom:0;padding:24px;border-top:3px solid #A03010;position:relative;">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px;">
            ...
            <span style="font-size:9px;font-weight:800;background:#FFF0EC;color:#A03010;padding:2px 8px;border-radius:10px;">+{{ $stats['today_slots_added'] }} Today</span>
        </div>
        <div style="font-size:11px;font-weight:700;text-transform:uppercase;color:#aaa;letter-spacing:0.8px;">Jadwal Tersedia</div>
        <div style="font-size:30px;font-weight:800;color:#111;margin-top:4px;">{{ $stats['available_slots'] }} Slot</div>
    </div>
    ...
    <div class="white-card" style="margin-bottom:0;padding:24px;border-top:3px solid #2E7D32;position:relative;">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px;">
            ...
            <span style="font-size:9px;font-weight:800;background:#E8F5E9;color:#2E7D32;padding:2px 8px;border-radius:10px;">Active</span>
        </div>
        <div style="font-size:11px;font-weight:700;text-transform:uppercase;color:#aaa;letter-spacing:0.8px;">Total Booking</div>
        <div style="font-size:30px;font-weight:800;color:#111;margin-top:4px;">{{ $stats['total_bookings'] }} User</div>
    </div>
    ...
    <div class="white-card" style="margin-bottom:0;padding:24px;border-top:3px solid #FF9800;position:relative;">
        ...
        <div style="font-size:30px;font-weight:800;color:#111;margin-top:4px;">{{ $stats['sessions_today'] }} Sesi</div>
    </div>
    ...
    <div class="white-card" style="margin-bottom:0;padding:24px;border-top:3px solid #555;position:relative;">
        ...
        <div style="font-size:30px;font-weight:800;color:#111;margin-top:4px;">{{ $stats['active_chats'] }} Sesi</div>
    </div>
</div>
    ...
    <div>
        ...
        <div class="white-card" style="border-radius:16px;padding:26px;margin-bottom:24px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
                ...
                <a href="{{ route('chef.consultations.index') }}" style="color:#A03010;font-size:13px;font-weight:700;display:flex;align-items:center;gap:4px;">Lihat Semua <i class="fas fa-arrow-right"></i></a>
            </div>
                ...
                <div style="background:#FFF;border:1px solid #F0F0F0;border-radius:14px;padding:16px 20px;display:flex;align-items:center;justify-content:space-between;">
                    <div style="display:flex;align-items:center;gap:16px;">
                        <div style="width:50px;height:50px;background:#FFF;border-radius:10px;border:1px solid #FFE0D8;display:flex;flex-direction:column;align-items:center;justify-content:center;line-height:1.2;">
                            <span style="font-size:16px;font-weight:800;color:#A03010;">
                                {{ $c->schedule ? \Carbon\Carbon::parse($c->schedule->available_date)->format('d') : '' }}
                            </span>
                            <span style="font-size:9px;font-weight:700;color:#aaa;text-transform:uppercase;">
                                {{ $c->schedule ? strtoupper(\Carbon\Carbon::parse($c->schedule->available_date)->isoFormat('MMM')) : '' }}
                            </span>
                        </div>
                        <div>
                            <h4 style="font-size:15px;font-weight:800;color:#111;margin:0;">{{ $c->topic ?? 'VIP Consultation Session' }}</h4>
                            <p style="font-size:12px;color:var(--text-m);margin-top:4px;">
                                User: {{ $c->user->name }} • {{ $c->schedule ? substr($c->schedule->available_time_start, 0, 5) . ' - ' . substr($c->schedule->available_time_end, 0, 5) : '' }}
                            </p>
                        </div>
                    </div>
                    <div style="display:flex;align-items:center;gap:12px;">
                        @if($c->status === 'ongoing' || $c->status === 'confirmed')
                            <a href="{{ route('chef.consultations.chat', $c) }}" class="btn btn-primary" style="background:#A03010;border:none;font-size:12px;padding:8px 18px;border-radius:20px;">Mulai Chat</a>
                        ...
                        @else
                            <span style="background:#E8F5E9;color:#2E7D32;font-size:11px;font-weight:700;padding:6px 14px;border-radius:20px;">{{ ucfirst($c->status) }}</span>
                        @endif
                    </div>
                </div>
                @empty
                <div style="text-align:center;padding:30px 20px;color:#aaa;font-size:14px;font-weight:500;">
                    Belum ada jadwal konsultasi terdekat.
                </div>
                @endforelse
            </div>
        </div>
        ...
        <div style="background:#FFC1B2;border-radius:20px;padding:28px 36px;display:flex;justify-content:space-between;align-items:center;position:relative;overflow:hidden;box-shadow:0 8px 24px rgba(255,90,54,0.15);min-height:175px;">
            <div style="max-width:300px;z-index:2;">
                ...
                <p style="font-size:12px;color:#A03010;font-weight:600;line-height:1.5;margin-bottom:18px;">Ada pesan belum terbaca dari VIP kitchen users yang menunggu saran bumbu Anda.</p>
    ...
    <div>
        ...
        <div class="white-card" style="border-radius:16px;padding:26px;margin-bottom:24px;">
            ...
            <div style="display:flex;flex-direction:column;gap:16px;margin-bottom:20px;">
                @forelse($vipBookings as $v)
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <div style="display:flex;align-items:center;gap:12px;">
                        <img src="{{ $v->user->avatar_url }}" style="width:38px;height:38px;border-radius:50%;object-fit:cover;border:1px solid #EEE;" alt="">
                        <div>
                            <h4 style="font-size:13px;font-weight:800;color:#333;margin:0;">{{ $v->user->name }}</h4>
                            <p style="font-size:10px;font-weight:700;color:#2E7D32;margin-top:2px;">VIP Member</p>
                        </div>
                    </div>
                    <a href="{{ route('chef.consultations.chat', $v) }}" style="color:#A03010;font-size:16px;"><i class="far fa-comment-dots"></i></a>
                </div>
                @empty
                <div style="color:#aaa;font-size:13px;text-align:center;padding:10px 0;">
                    Belum ada booking VIP aktif.
                </div>
                @endforelse
            </div>
