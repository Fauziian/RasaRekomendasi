...
class Recipe extends Model
{
    ...
    {
        $slug = Str::slug($title);
        // Find existing slugs that start with the base slug (DB-agnostic)
        $matches = static::where('slug', 'like', $slug . '%')->pluck('slug')->toArray();

        if (!in_array($slug, $matches)) {
            return $slug;
        }
